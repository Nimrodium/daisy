...
use termion::{color::DetectColors, event::Key, input::TermRead, raw::IntoRawMode};
...
pub fn main() -> Result<(), std::io::Error> {
    ...
    */
    ...
    if args.iter().any(|s| s == "--help") {
        ...
        let t = FormattedText::new(format!("Daisy v{}\n", env!("CARGO_PKG_VERSION")));
        t.write(&context, &mut stdout)?;
        return Ok(());
            ...
            if let Key::Char(q) = c.as_ref().unwrap() {
                match q {
                    '\n' => {
                        ...
                        if in_str.trim() == "quit" || in_str.trim() == "exit" {
                            break 'outer;
                        } else {
