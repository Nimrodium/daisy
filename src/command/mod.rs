...
#[inline(always)]
pub fn do_command(context: &mut Context, s: &String) -> FormattedText {
    let args: Vec<&str> = s.split(" ").collect();
    let first = args[0];
    ...
    match first {
        ...
        "vars" => {
            ...
            if v.len() + f.len() == 0 {
                return FormattedText::new("You have not defined any variables\n\n".to_string());
            }
        ...
        "del" | "delete" => {
            ...
            return match r {
                ...
                Err(()) => FormattedText::new(format!("[c]{v}[n] [e]isn't a variable.[n]\n\n",)),
            };
        }
