...
fn format_map_ansi(s: &str) -> Option<String> {
    Some(match s {
        ...
        "t" => {
            // Title text (should be cyan)
            format!(
                "{}{}",
                color::Fg(color::AnsiValue(6)),
                color::Bg(color::Reset)
            )
        }
        "a" => {
            // Colored text (should be pink)
            format!(
                "{}{}",
                color::Fg(color::AnsiValue(5)),
                color::Bg(color::Reset)
            )
        }
        "e" => {
            // Error titles (should be red)
            format!(
                "{}{}",
                color::Fg(color::AnsiValue(1)),
                color::Bg(color::Reset)
            )
        }
        "c" => {
            // Console text (inverted black on white)
            format!(
                "{}{}",
                color::Fg(color::AnsiValue(0)),
                color::Bg(color::AnsiValue(7))
            )
            // format!(
            //     "{}{}",
            //     color::Fg(color::AnsiValue(5)),
        ...
        "p" => {
            // Input prompt (how ==> is styled) (should be blue)
            format!(
                "{}{}",
                color::Fg(color::AnsiValue(4)),
                color::Bg(color::Reset)
            )
        }
        "s" => {
            // Repeat prompt (how => is styled) (should be pink)
            format!(
                "{}{}",
                color::Fg(color::AnsiValue(5)),
                color::Bg(color::Reset)
        ...
        "r" => {
            // Result prompt (how = is styled) (should be green)
            format!(
                "{}{}",
                color::Fg(color::AnsiValue(2)),
                color::Bg(color::Reset)
            )
        }
...
fn format_map_full(s: &str) -> Option<String> {
    Some(match s {
        ...
        "c" => {
            ...
            format!("{}{}", color::Fg(color::LightBlue), style::Italic)
        }
        "p" => {
            // Input prompt (how ==> is styled)
            format!("{}{}", color::Fg(color::Blue), style::Bold)
        }
        "s" => {
            // Repeat prompt (how => is styled)
            format!("{}{}", color::Fg(color::Magenta), style::Bold)
        }
        "r" => {
            // Result prompt (how = is styled)
                ...
                ']' => {
                    if !reading {
                        ...
                        if f.is_some() {
                            ...
                            out.push_str(&format!("{}", cursor::Right(n),));
                        } else {
                            out.push_str(&word);
                        }
